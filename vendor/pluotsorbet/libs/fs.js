...
var fs = (function() {
  ...
  function open(path, cb) {
    ...
    // PATCH(j2me-nx-port): 同步打开。上游用 FileReader 异步读 Blob，依赖
    // 浏览器事件循环；我们的 Blob 是内存字节数组，可直接同步读取。异步回调
    // 链路（setTimeout -> EventTarget -> Scheduler）在 nx.js 移植层与 VM 的
    // unwind 机制有交互问题（RMS 打开时 "Must have unwound frames" 断言失败，
    // 线程状态损坏）。同步回调让 asyncImpl 的 promise 立即 settle，恢复可靠。
    var bytes = (record.data && typeof record.data.getBytes === "function")
      ? record.data.getBytes()
      : new Uint8Array(0);
    openedFiles.set(++lastId, {
      dirty: false,
      path: path,
      buffer: new FileBuffer(new Int8Array(bytes.slice().buffer)),
      mtime: record.mtime,
      size: record.size,
      position: 0,
      record: record,
