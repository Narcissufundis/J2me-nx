...
function inflate(bytes, uncompressed_len) {
  // PATCH(j2me-nx-port perfZ25)：耗时账本。zip entry 解压是纯 JS inflate
  // （每字节循环），游戏入口要解压几百个 .class + 资源，是"进入游戏卡几秒"
  // 的头号嫌疑。这里记 {次数, 毫秒, 输出字节}，宿主每 10s 汇总成 [cost]。
  var __t0 = (typeof jsGlobal !== 'undefined' && jsGlobal.__costAdd) ? Date.now() : 0;
  var __out = _inflate(bytes, uncompressed_len);
  if (__t0 && typeof jsGlobal !== 'undefined' && jsGlobal.__costAdd) {
    jsGlobal.__costAdd('解压', Date.now() - __t0, __out ? __out.length : 0);
  }
  return __out;
}

function _inflate(bytes, uncompressed_len) {
  var bytesPos = 0;
  ...
  function getBits(bits) {
    var b;
    while (codeSize < bits) {
      // PATCH(j2me-nx-port): 越界防护。原版在此处 undefined|0=0 无限喂零，
      // 损坏/截断的 entry 会构造出永不终止的解压循环 = 主线程永久堵死。
      // 注意必须用 > 而非 >=：合法 deflate 流允许在最后一个字节后借一次
      // "幻影零位"解出块结束码（codeSize 不足 8 时），多喂一次后 bytesPos
      // 即超界，再读就抛——不会无限喂零。
      if (bytesPos > bytes.length) throw new Error('flate: 数据耗尽（损坏的 zip entry）');
      b = bytes[bytesPos++] | 0;
      codeBuf |= b << codeSize;
      codeSize += 8;
    }
  ...
  function readBlock() {
    ...
    if (hdr == 0) { // uncompressed block
      ...
      // PATCH(j2me-nx-port) 2026-09-20 二轮：原版这 5 处 new Error 全部缺
      // throw（死代码），损坏的 stored 块会带着 NaN blockLen 继续跑。
      // 统一改为真抛出，与 getBits/getCode 的防护语义一致。
      ...
      // 防御：blockLen 声称超出输出缓冲 → 数据损坏，拒绝继续
      if (bufferLength + blockLen > buffer.length)
        throw new Error('flate: stored 块溢出输出缓冲');
    ...
    if (hdr == 1) { // compressed block, fixed codes
      ...
      while (i < codes) {
        ...
        // PATCH(j2me-nx-port): 重复越界 = 数据损坏（typed array 会静默丢弃
        // 越界写导致 i 失控），直接拒绝。
        if (i + repeatLength > codes)
          throw new Error('flate: code 长度重复越界');
    ...
    while (true) {
      ...
      if (code1 < 256) {
        // PATCH(j2me-nx-port): 输出缓冲写满仍在产出字面量 → uncompressed_len
        // 与流不符。原版会静默丢弃越界写并空转到数据耗尽。
        if (bufferLength >= buffer.length)
          throw new Error('flate: 输出溢出（uncompressed_len 与流不符）');
      ...
      if (code1 === undefined)
        throw new Error('flate: 非法距离码');
      code2 = code1 >> 16;
      if (code2 > 0)
        code2 = getBits(code2);
      var dist = (code1 & 0xffff) + code2;
      // PATCH(j2me-nx-port): 回引用越界（dist 指到输出起点之前 / 输出溢出）
      // = 流已损坏。原版读负索引得 undefined→0，静默产出垃圾数据。
      if (dist === 0 || dist > bufferLength || bufferLength + len > buffer.length)
        throw new Error('flate: 回引用越界（损坏的 deflate 流）');
