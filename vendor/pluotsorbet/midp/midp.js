...
var MIDP = (function() {
  ...
  function updatePhysicalScreenSize(w, h) {
    // PATCH(j2me-nx-port): 支持宿主直写物理屏尺寸（幂等）。
    // 软重启跨分辨率时，物理屏与画布的同步原本完全依赖一次性 resize 事件
    // （env-prelude 派发 → onWindowResize 重算 physicalScreen）；事件链任何
    // 一次失手，updateCanvas 就会在游戏每次 setFullScreen0 时把设备画布
    // 重置回旧值，与宿主呈现层的漂移自愈互相"打架"。直写绕开事件依赖：
    // 同值调用零副作用，异值由 updateCanvas 派发一次 canvasresize。
    if (typeof w === 'number' && typeof h === 'number' && w > 0 && h > 0 &&
        w <= 4096 && h <= 4096) {
      if (physicalScreenWidth !== w || physicalScreenHeight !== h) {
        physicalScreenWidth = w;
        physicalScreenHeight = h;
    ...
    if (!config.autosize || /no|0/.test(config.autosize)) {
      // PATCH: 回落分支同样拒绝零尺寸（display 元素缺失/未布局时 clientWidth 为 0）
      var dispEl = document.getElementById('display');
      if (dispEl && dispEl.clientWidth > 0 && dispEl.clientHeight > 0) {
        physicalScreenWidth = dispEl.clientWidth;
        physicalScreenHeight = dispEl.clientHeight;
      }
    }
  }
  ...
  function updateCanvas() {
    ...
    // PATCH(j2me-nx-port): 零/负尺寸绝不上屏——nx.js 对画布任何赋值都重建
    // surface 清空内容，写成 176x0 就等于把游戏画面永久抹掉（三国志3 实机案）。
    if (newHeight > 0 && newWidth > 0 &&
        (newHeight != deviceCanvas.height || newWidth != deviceCanvas.width)) {
  ...
  function onWindowResize(evt) {
    ...
    // PATCH(j2me-nx-port): nx.js 下 window.outerWidth/outerHeight 可能为 0
    //（实机 2026-09-21 三国志3：宿主自愈重设画布 → nx.js 派发 window resize →
    // 此处重算出 physicalScreen=176x0 → updateCanvas 每秒把画布写成高度 0
    // （nx.js 对画布任何赋值都清内容）→ 与宿主呈现层自愈互相打架 → 永远黑屏）。
    // 无效尺寸一律忽略，物理屏尺寸以宿主 updatePhysicalScreenSize 直写为准。
    if (!(newPhysicalScreenWidth > 0 && newPhysicalScreenHeight > 0 &&
          newPhysicalScreenWidth <= 4096 && newPhysicalScreenHeight <= 4096)) {
      return;
    }
  ...
  // PATCH(j2me-nx-port): 宿主（Switch 手柄轮询）直发键码的入口。
  // 绕开合成 DOM 事件链（Event 构造/window 别名/捕获等环节都可能缺失），
  // 且天然尊重 suppressKeyEvents（GameCanvas 模式）。
  window.__sendKeyPress = sendKeyPress;
  window.__sendKeyRelease = sendKeyRelease;

  // PATCH(j2me-nx-port): 宿主把"输入法文本"送进 MIDlet（游戏内打字）。
  //
  // 依据（反汇编 java/classes.jar 的 com/sun/midp/lcdui/DisplayEventListener）：
  //     if (nativeEvent.intParam1 == 4) consumer.handleInputMethodEvent(stringParam1);
  //     else                            consumer.handleKeyEvent(intParam1, intParam2);
  // 也就是说 **LCDUI 的按键事件里根本没有字符槽位**（只有 eventType + keyCode），
  // 文字只能从 intParam1 == 4 这条"输入法事件"通道进。宿主弹出系统键盘拿到字符串后，
  // 用这个入口送进去，当前获得焦点的 TextBox/TextField 就会收到。
  function sendInputMethodText(text) {
    if (text === null || text === undefined) return;
    FG.sendNativeEventToForeground({
      type: KEY_EVENT,
      intParam1: 4,             // 4 = 输入法文本事件（DisplayEventListener 里那个分支）
      intParam2: 0,
      stringParam1: String(text),
    }, true);
  }
  window.__sendInputMethodText = sendInputMethodText;
  ...
  // PATCH(j2me-nx-port): 上游缺 handleFatalError 实现——事件泵 catch 到
  // Throwable 后调用它会因 native 缺失二次抛错，直接杀死事件泵线程：
  // 游戏画面继续刷但输入永久失效（流星"是否开启音乐"冻结案）。这里
  // 吞掉致命错误保活事件泵，仅落盘留痕。
  Native["com/sun/midp/events/EventQueue.handleFatalError.(Ljava/lang/Throwable;)V"] = function(addr, throwableAddr) {
    var name = "Throwable";
    try {
      if (throwableAddr) name = J2ME.getClassInfo(throwableAddr).getClassNameSlow();
    } catch (eName) { /* 忽略 */ }
    console.error("[fatal-event] 事件泵致命错误(已吞掉保活): " + name);
  };
  ...
  var keyMap = {
    // PATCH(j2me-nx-port): 除原浏览器键码外，补诺基亚标准键码（Switch 端
    // switch-input.js 直接发 -1..-5 / 数字键）。真实设备 D-pad 上报负键码。
    1: 119, // UP
    2: 97, // LEFT
    5: 100, // RIGHT
    6: 115, // DOWN
  ...
  // 诺基亚键码版（同名 game action 的设备键码，优先级：Nokia 优先）
  var keyMapNokia = {
    1: -1, // UP
    2: -3, // LEFT
    5: -4, // RIGHT
  ...
  Native["javax/microedition/lcdui/KeyConverter.getKeyCode.(I)I"] = function(addr, key) {
    // PATCH(j2me-nx-port): 真机（Switch）用诺基亚键码反查，仿真保持浏览器键码
    if (typeof window.__nokiaKeyMap !== 'undefined' && keyMapNokia[key] !== undefined) {
      return keyMapNokia[key];
    }
    return keyMap[key] || 0;
  };
  ...
  Native["javax/microedition/lcdui/KeyConverter.getKeyName.(I)Ljava/lang/String;"] = function(addr, keyCode) {
    // PATCH(j2me-nx-port): 先查宿主按"按键机型"登记的表。
    // 软键码各厂商不同（诺基亚 -6/-7、摩托罗拉 -21/-22…），而游戏常靠
    // getKeyName(code) 反查"这个码是不是软键"（SO 上流传的机型适配代码就是这么干的），
    // 所以宿主把各机型的软键码都登记进 globalThis.__softKeyNameMap。
    try {
      var extra = (typeof globalThis !== "undefined") ? globalThis.__softKeyNameMap : null;
      if (extra) {
        var key = String(keyCode);
        if (Object.prototype.hasOwnProperty.call(extra, key) && extra[key]) {
          return J2ME.newString(extra[key]);
        }
      }
    } catch (e) { /* 查表失败不影响原有行为 */ }
  ...
  var gameKeys = {
    // PATCH(j2me-nx-port): 补诺基亚标准键码（-1..-5 方向/确认、
    // 数字键 2/4/6/8/5 同时是方向/确认，真实键盘机的惯例）；
    // 原 w/a/s/d 浏览器键码保留（Node 仿真用）。
    "-1": 1,  // UP
    "-2": 6,  // DOWN
    "-3": 2,  // LEFT
    "-4": 5,  // RIGHT
    "-5": 8,  // FIRE
    // PATCH(j2me-nx-port): 摩托罗拉"OK 键" -20 也是 FIRE。
    // 实机摩托罗拉的确认/OK 硬件键报的 keyCode 就是 -20（用户提供的键表：
    // 左软件 -21 / 右软件 -22 / OK -20），而按 MIDP 规范 getGameAction() 必须
    // 把确认键翻成 FIRE(8)。宿主在"摩托罗拉"按键机型下发 -20，这里补上映射，
    // 于是一份用 getGameAction()==FIRE 判断的游戏在该机型下也能正常确认。
    "-20": 8, // FIRE（摩托罗拉 OK 键）
    // PATCH(j2me-nx-port): QWERTY 横屏机（E52/E63）把**数字键报成字符码**
    // （数字键底下那层字母的 ASCII：2=t(116) 4=f(102) 6=h(104) 8=b(98) 5=g(103)）。
    // 宿主"诺基亚横屏"档发这些字符码，这里按 MIDP 规范把方向和确认翻出来，
    // 否则用 getGameAction()==UP/LEFT/… 判断的游戏在该档下会失灵。
    116: 1,   // 't' = 数字 2 → UP
    102: 2,   // 'f' = 数字 4 → LEFT
    104: 5,   // 'h' = 数字 6 → RIGHT
    98: 6,    // 'b' = 数字 8 → DOWN
