...
Media.ContentTypes = {
    // PATCH(j2me-nx-port): MIDI 走纯 JS 合成器（midi-synth.js），Manager.createPlayer
    // 在 Java 层用本白名单校验，缺了直接抛 MediaException。
    memory: ["audio/midi", "audio/mid"],
...
Media.contentTypeToFormat = new Map([
    ...
    ["video/mp4", "MPEG4"],
    ["video/webm", "WebM"],
    // PATCH(j2me-nx-port): MIDI 走纯 JS 合成器（见 midi-synth.js），不经 FFmpeg
    ["audio/midi", "mid"],
    ["audio/mid", "mid"],
]);
...
// PATCH(j2me-nx-port perfZ24): 媒体探针限流器（说明见 realize/start 两处调用点）。
// 每类前 3 次完整落盘 → 之后每 25 次留一条 → 每 ≥10s 一行批量汇总（不写卡）。
var mediaMarkStats = Object.create(null);

function mediaMark(key, line) {
    try {
        if (typeof jsGlobal.__sdMark !== "function") return;
        var st = mediaMarkStats[key];
        if (!st) { st = mediaMarkStats[key] = { n: 0, logged: 0, last: 0 }; }
        st.n++;
        if (st.logged < 3) {
            st.logged++;
            jsGlobal.__sdMark(line);
            return;
        }
        if (st.n % 25 === 0) {
            jsGlobal.__sdMark(line);
        ...
        if (now - st.last > 10000) {
            st.last = now;
            if (typeof jsGlobal.__sdLog === "function") {
                jsGlobal.__sdLog("[media] " + key + " 累计 " + st.n + " 次（探针已限流，每 25 次留一条）");
            }
        }
    } catch (e) { /* 探针绝不能引发二次故障 */ }
}
...
PlayerContainer.prototype.realize = function(contentType) {
    // PATCH(j2me-nx-port): 挂起定位探针——KGame 类引擎开机即建播放器，
    // 主线程若堵死（rAF/macrotask 全停）最后一条 [media] 标记即死点
    //（2026-09-21 流星蝴蝶幻剑录2：启动 ~1s 后主线程阻塞黑屏）。
    //
    // PATCH(perfZ24)：**限流**（与 midiMark 同一套策略）。__sdMark = 一次同步写卡，
    // 而"每个音效建一个 Player"的游戏会把这条探针变成"每个声音卡一下"。
    // 规则：每类前 3 次完整落盘 → 之后每 25 次留一条（保住"最后一条=死点"语义）
    //      → 每 ≥10s 一行批量汇总。
    mediaMark("realize enter fmt=" + this.mediaFormat,
        "[media] realize enter fmt=" + this.mediaFormat +
        " ct=" + contentType + " url=" + this.url);
    return new Promise((function(resolve, reject) {
        if (contentType) {
        ...
        if (this.mediaFormat === "mid") {
            // PATCH(j2me-nx-port): MIDI → 纯 JS 合成器（midi-synth.js）
            this.player = new Media.MidiPlayer(this);
            this.player.realize().then(resolve, reject);
        } else if (Media.supportedAudioFormats.indexOf(this.mediaFormat) !== -1) {
...
PlayerContainer.prototype.start = function() {
    // PATCH(perfZ24)：同样限流（音效每个都 start，是最容易被写卡拖慢的一条）
    mediaMark("start fmt=" + this.mediaFormat,
        "[media] start " + (this.player && this.player.constructor ? this.player.constructor.name : "?") +
        " fmt=" + this.mediaFormat);
    this.player.start();
};
...
function TonePlayer() {
    // PATCH(j2me-nx-port 2026-09-22): 共享全局 AudioContext（原 per-instance
    // new 会泄漏 SDL 设备且每次分配几十~几百 ms，实机卡顿元凶之一）。
    this.audioContext = (typeof MidiPlayer !== "undefined" && MidiPlayer.getSharedContext)
        ? MidiPlayer.getSharedContext()
        : new AudioContext();

    // PATCH(j2me-nx-port): nx.js 未实现 createOscillator（throw Method not
    // implemented.），改用宿主 __toneBank 的可循环 PCM 波表 + BufferSource。
...
TonePlayer.prototype.playTone = function(note, duration, volume) {
    ...
    // PATCH(j2me-nx-port): 原来是 osc.detune.value = (note-69+21)*100（默认
    // 440Hz 振荡器 + 音分偏移）。nx.js 没有 OscillatorNode，这里直接按公式
    // 算出频率，取 __toneBank 的循环波表（整数周期，loop 无缝）。
    var bank = (typeof globalThis !== "undefined" && globalThis.__toneBank) || null;
    if (!bank) {
        return;
    }
    var freq = 440 * Math.pow(2, (note - 69 + 21) / 12);
    this.oscillator = this.audioContext.createBufferSource();
    this.oscillator.buffer = bank.loop(this.audioContext, "sine", freq);
