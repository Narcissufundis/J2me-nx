...
module J2ME.Bytecode {
  ...
  export class Block {
    ...
    public isInnerLoopHeader() {
      // PATCH(j2me-nx-port): loopID<0 表示超过 32 个循环未分配位图（见 makeLoopHeader），按最外层头处理
      if (this.loopID < 0) {
        return false;
      }
    ...
    private makeLoopHeader(block: Block) {
      if (!block.isLoopHeader) {
        ...
        if (this._nextLoop >= 32) {
          // PATCH(j2me-nx-port): 不再因 >32 个循环拒编。loops 位图仅被 OSR 入口点
          // 计算（enableOSR=false 时不消费）与可归约性检查（本移植的 switch 派发
          // Relooper 支持任意 CFG）使用。超限的循环头 loopID 保持 -1、不占位图位。
          return;
        }
    ...
    private fixLoopBits() {
      ...
      function _fixLoopBits(block: Block) {
        if (block.visited) {
          ...
          if (block.isLoopHeader) {
            // PATCH(j2me-nx-port): loopID<0 的循环头不参与位图清位
            return block.loopID >= 0 ? (block.loops & ~(1 << block.loopID)) : block.loops;
          } else {
            return block.loops;
          }
        }
        ...
        if (block.isLoopHeader && block.loopID >= 0) {
