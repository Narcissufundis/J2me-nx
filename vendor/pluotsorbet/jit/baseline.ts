module J2ME {
  ...
  export class BaselineCompiler {
    ...
    classInfoSymbol(classInfo: ClassInfo): string {
      ...
      // PATCH(j2me-nx-port): 缺项不再 assert 拒编，就地补注册（索引数组即最终传给
      // linkMethodSource 的 referencedClasses，生成期补入与 lookup 期加入等价）。
      if (id < 0) {
        id = this.referencedClasses.length;
        this.referencedClasses.push(classInfo);
      }
    ...
    methodInfoSymbol(methodInfo: MethodInfo): string {
      ...
      // PATCH(j2me-nx-port): 同 classInfoSymbol，缺项补注册不拒编。
      if (id < 0) {
        id = this.referencedClasses.length;
        this.referencedClasses.push(methodInfo.classInfo);
      }
    ...
    emitNewTypeArray(typeCode: number) {
      ...
      // PATCH(probe): 原发射 MA(...)（gcMallocAtomic 直连，探针不可见），
      // 改 PA(bytes, classId) = probeArrayAlloc：按类记账后同样调
      // gcMallocAtomic，语义不变。
      this.emitPush(Kind.Reference, "PA(" + Constants.ARRAY_HDR_SIZE + "+na*" + arrayClassInfo.bytesPerElement + "," + arrayClassInfo.id + ")");
