...
module J2ME {
  ...
  export class ClassInfo extends ByteStream {
    ...
    public complete() {
      ...
      // j2me-nx-port: 接口也必须做字段布局。上游假设接口静态字段只是编译期常量
      // （javac 会内联、不占静态槽位），但混淆器（proguard 等）会把非常量静态表
      // 塞进接口 —— 若跳过 buildFTable，所有静态字段 byteOffset 保持 0，全部
      // 叠在同一地址互相覆盖（Diamond Rush 的 AIOOBE:0 根因）。
      this.buildFTable();
  ...
  export class ArrayClassInfo extends ClassInfo {
    ...
    constructor(elementClass: ClassInfo) {
      ...
      (<any>this).superClassName = CLASSES.java_lang_Object.getClassNameSlow(); // PATCH(j2me-nx-port): TS2.9 readonly 检查
      this.depth = 1;
    }
  }
