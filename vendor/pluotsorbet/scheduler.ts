      ...
      function run() {
        ...
        try {
          ...
          while (runningQueue.length) {
            ...
            current = null;
            // GC(20260922-gc1)：安全点——execute() 返回后所有线程帧已同步进
            // 堆（解释器帧在 threadData、JIT 帧在 preempt 时物化的 bailout
            // 帧），JS 栈上无 Java 引用在途。堆余量不足时在此回收。
            if (ASM.__gcPending && ASM.__gcPending()) {
              try {
                var gcStats = ASM.__collect(J2ME.collectGarbage);
                if (gcStats && jsGlobal && jsGlobal.__gcReport) {
                  jsGlobal.__gcReport(gcStats);
                }
              } catch (eGC) { /* native-heap 侧已自禁用，这里兜底 */ }
            }
          }
        } finally {
