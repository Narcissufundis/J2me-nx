...
jars.forEach(function(jar) {
  loadingMIDletPromises.push(load(jar, "arraybuffer").then(function(data) {
    ...
    // PATCH(j2me-nx-port): 原生崩溃诊断——游戏 jar 入库即时落盘标记
    if (typeof jsGlobal.__sdMark === "function") {
      jsGlobal.__sdMark(jar + " -> JARStore (" + (data ? data.byteLength : 0) + "B)");
    }
    // PATCH(j2me-nx-port perfZ25): 入口时间轴
    if (typeof jsGlobal.__enterMark === "function") {
      jsGlobal.__enterMark("入库");
    }
  }));
});
...
function start() {
  ...
  function run() {
    ...
    // PATCH(j2me-nx-port): 原生崩溃诊断——JVM 启动前即时落盘标记
    if (typeof jsGlobal.__sdMark === "function") {
      jsGlobal.__sdMark("jvm startIsolate0 " + config.main);
    }
    // PATCH(j2me-nx-port perfZ25): 入口时间轴
    if (typeof jsGlobal.__enterMark === "function") {
      jsGlobal.__enterMark("isolate启动");
    }
    jvm.startIsolate0(config.main, config.args);
    if (typeof jsGlobal.__sdMark === "function") {
      jsGlobal.__sdMark("jvm startIsolate0 返回（同步初始化完成）");
    }
    if (typeof jsGlobal.__enterMark === "function") {
      jsGlobal.__enterMark("isolate返回");
    }
  }
}
