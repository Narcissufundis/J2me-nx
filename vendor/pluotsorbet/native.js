...
Native["java/lang/System.getProperty0.(Ljava/lang/String;)Ljava/lang/String;"] = function(addr, keyAddr) {
    ...
    case "microedition.encoding":
        // PATCH(j2me-nx-port) 2026-09-23：上游这里是 "UTF-8"（照 Nokia Asha 503 的
        // 浏览器运行时写的），但**中文 J2ME 游戏把文本按 GBK 存在 jar/存档里**，
        // 而 `new String(byte[])` / `getBytes()` 走的就是这个"平台默认编码"。
        // 实机「Forgotten Warrior 丢文字」的真凶即此：游戏的 1.sn 剧情文本是 GBK，
        // 被当 UTF-8 解 → 每个汉字变成 U+FFFD（实测 91 个替换字符），整段文字没了。
        // 改成 GBK 等于**模拟一台中文手机**（GBK 是 ASCII 超集，纯英文数据不受影响）。
        // 另有两条兜底，见：
        //   * java/cldc1.1.1/com/sun/cldc/i18n/Helper.java —— `new String(byte[])`
        //     先做严格 UTF-8 判定，是合法 UTF-8 就按 UTF-8（照顾 UTF-8 存档的游戏）；
        //   * vendor/pluotsorbet/midp/conv.js —— nx.js 的 TextDecoder 只有 utf-8，
        //     GBK 解码靠自带的 gbk-table.js 映射表。
        value = "GBK";
        break;
...
Native["com/sun/cldc/io/ResourceInputStream.clone.(Ljava/lang/Object;)Ljava/lang/Object;"] = function(addr, sourceAddr) {
    ...
    setNative(objAddr, {
        // PATCH(j2me-nx-port perfZ22): **共享** data（只读），不再整份拷贝。
        // 旧实现 `new Uint8Array(sourceDecoder.data)` 会把整个资源复制一遍，而这个
        // native 只被 mark() 调用 —— 也就是说游戏每 mark() 一次，10MB 的 .rc 就整份
        // 复制一次（内存 + 时间双重浪费）。reset() 需要的只是 pos，data 本身从不被写。
        data: sourceDecoder.data,
...
// PATCH(j2me-nx-port perfZ22): 资源流"快进"必须 O(1)。
// 背景（2026-09-24 实机，重装机兵2-火线突击"进图卡住"）：
//   ResourceInputStream 只实现了 read()/read(byte[],int,int)，**没有覆盖 skip()**
//   → 继承 java.io.InputStream.skip 的**逐字节**实现，而每个字节都是一次 native
//   调用（readByte）。实机采样榜：[hot-top10] java/io/InputStream.skip=22% +
//   com/sun/cldc/io/ResourceInputStream.read=19% —— 41% 的解释器时间耗在这条路上；
//   [vm-sample] 还抓到当次调用的 locals[1]=0xfaca，即**一次 skip 要跳过 64202 字节**
//   = 6.4 万次 native 调用。游戏从 .rc 资源包里按偏移取图（每张图先 skip 到记录
//   位置），于是一个地图的资源加载从"秒级"变成"分钟级"——玩家看到的就是卡死。
// 修法：解码器就是一个 {data, pos} 游标，**直接把 pos 推到位**（O(1)），
//       skip 的语义（返回实际跳过字节数、到流尾提前结束）保持不变。
Native["com/sun/cldc/io/ResourceInputStream.skipBytes.(Ljava/lang/Object;J)J"] =
function(addr, fileDecoderAddr, nLow, nHigh) {
    var handle = NativeMap.get(fileDecoderAddr);
    if (!handle) {
        return J2ME.returnLongValue(0);
    }
    var n = (nLow >>> 0) + nHigh * 4294967296;   // long 低/高 32 位拼回 number
    if (!(n > 0)) {
        return J2ME.returnLongValue(0);
    }
    var remaining = handle.data.length - handle.pos;
    var skipped = (n < remaining) ? n : remaining;
    handle.pos += skipped;
    return J2ME.returnLongValue(skipped);
};
