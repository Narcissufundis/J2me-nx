...
process.on('unhandledRejection', (reason) => {
  ...
  try {
    const d = globalThis.__jitDump;
    if (d) {
      writeFileSync(join(root, 'bld', 'jit-dump-' + d.key.replace(/\W+/g, '_') + '.js'), '// err: ' + d.err + '\n// key: ' + d.key + '\n' + d.body);
      console.log('[jit-debug] body saved: ' + d.key);
    } else { console.log('[jit-debug] no __jitDump'); }
  } catch (e) { console.log('[jit-debug] dump failed: ' + e); }
});
