...
// ************************************************************
    ...
    public boolean isShown() {
        boolean shown;
        synchronized (Display.LCDUILock) {
            shown = displayableLF.lIsShown();
        }
        // PATCH(j2me-nx-port): 协作式线程调度防饿死——KGame 等引擎在
        // run() 里无 sleep 忙等 isShown()，永远不交出执行权会让事件泵
        // （负责 setCurrent 完成态/前台通知）饿死，形成自锁。这里在
        // 结果为 false 时主动 yield 一次（必须在 LCDUILock 之外，VM
        // 不允许持 monitor yield），让其他线程有机会推进。
        if (!shown) {
            Thread.yield();
        }
        return shown;
