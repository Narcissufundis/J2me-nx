...
abstract class DisplayableLFImpl implements DisplayableLF {
    ...
    DisplayableLFImpl(Displayable d) {
        ...
        // PATCH(j2me-nx-port): phoneME 的 Display.initClass() 仅由
        // LCDUIEnvironment.initDisplayEventHandler() 调用，而后者在
        // PluotSorbet 中是死代码，Display.WIDTH/HEIGHT 恒为 0。
        // 部分 MIDP-1.0 游戏（如诛仙伏魔录）在 MIDlet 构造器里就取屏宽高，
        // 会拿到 0 并在 createImage(0,0) 处崩溃。此处直接向原生
        // DisplayDevice 查询真实屏幕尺寸兜底。
        if (width <= 0 || height <= 0) {
            try {
                com.sun.midp.lcdui.DisplayDevice device =
                    new com.sun.midp.lcdui.DisplayDevice(1);
                int dw = device.getWidth();
                int dh = device.getHeight();
                if (width <= 0) {
                    width = dw;
                }
                if (height <= 0) {
                ...
                // 查询失败则维持原值，由后续 sizeChanged 流程修正
            }
        }
    }
