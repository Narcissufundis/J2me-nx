...
public abstract class InputStream {
    ...
    public long skip(long n) throws IOException {
        // PATCH(j2me-nx-port perfZ22): 原来的实现是"逐字节 read()"，而在我
        // 们的移植里每次 read() 都是一次 JS native 调用（μs 级），跳过一个
        // 64KB 的记录就是 6.4 万次调用 —— 实机表现为"进图卡死"。
        // 这里改成用 bulk 的 read(byte[],int,int) 分块消费：语义完全不变
        // （返回值仍是"实际跳过多少字节"，到流尾提前结束），但调用次数从
        // O(n) 降到 O(n/512)。子类若有更快的 skip（如 ResourceInputStream 的
        // O(1) 游标推进）会覆盖本方法，走不到这里。
        if (n <= 0) {
            return 0;
        }
        long m = n;
        // 小 skip 不值得为它分配缓冲（游戏里 skip(1..8) 很常见）
        if (n <= 8) {
            while (m > 0) {
                if (read() < 0) {
                    break;
        ...
        byte[] buf = new byte[(int) (n < 512 ? n : 512)];
        while (m > 0) {
            int want = (int) (m < buf.length ? m : buf.length);
