 ...
 */
    ...
    boolean isShown(DisplayableLF d) {
        ...
        // PATCH(j2me-nx-port): 上游判定 hasForeground && !paintSuspended
        // && (current == d)。hasForeground 只由 AMS 前台通知事件回执置位，
        // 而该回执要靠事件泵线程处理；KGame 等引擎的 UI 线程在 run() 里
        // 无 sleep 忙等 isShown()（真机靠抢占式调度），PluotSorbet 协作式
        // 线程下事件泵永远得不到执行权 → hasForeground 永假 → isShown
        // 永假 → 死锁黑屏。退化为"是否当前显示物"判定：setCurrent 同步
        // 置位 current，游戏启动即可通过。副作用仅限后台态判定不准，
        // 本移植单应用全屏无后台切换，可接受。
        // transitionCurrent 覆盖 setCurrentImpl 过渡窗口。
        return (current == d) || (transitionCurrent == d);
    }
 ...
 */
    ...
    class DisplayEventConsumerImpl implements DisplayEventConsumer {
        ...
        public void handleInputMethodEvent(String inputText) {
            // PATCH(j2me-nx-port 2026-09-23): 上游这里是 `current instanceof TextBox`，
            // 但本移植里 Display.current 是 **DisplayableLF**（TextBox 的 LF 就是 FormLFImpl，
            // 见 LFFactoryImpl.getTextBoxFormLF），于是那个判断**永远为假** ——
            // 宿主注入的输入法文字到此为止，TextBox 始终是空的
            // （仿真日志：宿主"已注入文本"、事件也进来了，但 current 是 FormLFImpl）。
            // 修法：先把 Displayable 取回来再判断。
            TextBox textBoxCopy = null;
            TextField fieldCopy = null;

            synchronized (LCDUILock) {
                Displayable cur = null;
                if (current != null) {
                    cur = current.lGetDisplayable();
                }
                if (cur instanceof TextBox) {
                    textBoxCopy = (TextBox) cur;
                } else if (cur instanceof Form) {
                    // PATCH(j2me-nx-port 2026-09-23 之三): 上游只处理 TextBox，
                    // 而 **Form + TextField** 是游戏取名/输入的另一种常见写法
                    // （例：D:\新建文件夹\1.jar 的 g.class：Form a + TextField b，
                    //   commandAction 里 b.getString() 取值）。不补这一支，
                    // 系统键盘拿到的字会在这里被丢掉。
                    // 选目标：优先"可编辑"的那个 TextField（跳过 UNEDITABLE）。
                    Form f = (Form) cur;
                    int n = f.size();
                    for (int i = 0; i < n; i++) {
                        Item it = f.get(i);
                        if (it instanceof TextField) {
                            ...
                            if ((tf.getConstraints() & TextField.UNEDITABLE) == 0) {
                                fieldCopy = tf;
                                break;
                            }
                        }
                ...
                if (textBoxCopy == null && fieldCopy == null) {
                    System.out.println("[textinput] 当前界面没有可写的文本框（" +
                            (cur == null ? "null" : cur.getClass().getName()) + "），输入丢弃");
                }
            }
            ...
            if (textBoxCopy != null) {
                // PATCH(j2me-nx-port 2026-09-23 之二): 上游这里是 insert（在光标处插入），
                // 但我们的"输入法文本"来自**整段**系统键盘输入 —— 玩家打出来的一整串
                // 就是他要的内容。很多游戏建 TextBox 时会先塞一个默认名
                //（例：D:\新建文件夹\1.jar 的取名流程），用 insert 会变成
                // "默认名+你打的名字"。所以这里改成整体替换。
                textBoxCopy.setString(inputText);
                // 写成功了要留一行 —— 与上面"没有可写的文本框"配对，实机日志里一眼能看出
                // 到底是"没找到框"还是"写进去了、游戏却没读"（2026-09-23 取名排查教训：
                // 少了这一行就只能靠猜）。
                System.out.println("[textinput] 已写入 TextBox，" + inputText.length() + " 字");
            } else if (fieldCopy != null) {
                // Form 里的 TextField：同样整体替换（理由同上）
                fieldCopy.setString(inputText);
                System.out.println("[textinput] 已写入 Form 里的 TextField，" + inputText.length() + " 字");
            }
