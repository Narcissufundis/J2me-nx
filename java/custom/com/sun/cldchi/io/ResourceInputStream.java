...
public class ResourceInputStream extends InputStream {
    ...
    /**
     * PATCH(j2me-nx-port perfZ22): 覆盖 skip()，走 native 的 O(1) 游标推进。
     *
     * <p>为什么必须覆盖：不覆盖就会继承 {@link InputStream#skip(long)} 的**逐字节**
     * 实现，而这里每一字节都是一次 native 调用。实机（重装机兵2-火线突击"进图卡住"）
     * 采样榜上 InputStream.skip 占 22%、ResourceInputStream.read 占 19%，且抓到单次
     * skip 的实参是 64202 字节 —— 一次取图就要 6.4 万次 native 调用，整个地图的资源
     * 加载从秒级退化到分钟级（表现为卡死）。
     *
     * @param n 要跳过的字节数
     * @return 实际跳过的字节数（到流尾会小于 n）
     * @exception IOException 流已关闭时抛出
     */
    public long skip(long n) throws IOException {
        ...
        return skipBytes(fileDecoder, n);
    }
    ...
    // PATCH(j2me-nx-port perfZ22): O(1) 快进（见上面 skip 的注释）
    private static native long skipBytes(Object fileDecoder, long n);
    /*
