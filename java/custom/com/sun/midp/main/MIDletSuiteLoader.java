...
public class MIDletSuiteLoader extends CldcMIDletSuiteLoader {
    ...
    protected void reportError(int errorCode, String details) {
        ...
        // 2026-09-21：错误详情显式打到 System.err（→ console.info → error.log）。
        // 之前只走 SystemAlert 弹窗，实机上用户看不到、日志里也没有，
        // 启动失败的根因（AHQSL / 暗黑启示录2 等）无从定位。
        try {
            System.err.println("[startup-error] code=" + errorCode
                + " details=" + details);
        } catch (Throwable tLog) { /* 日志绝不二次崩溃 */ }
